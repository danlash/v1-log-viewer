...
==== Build: 10.3.0.5 ====
";
		private const string FIRST_EXCEPTION = @"VersionOne.Eventing.EventProcessingException: Error processing event: 
Task:395766:1419992,Member:268089,Changed, created 4/19/2011 9:47:03 PM, previous audit: 1419991; 
[no event asset captured]. ---> System.Data.SqlClient.SqlException: The query processor could not start the necessary thread resources for parallel query execution.
   at System.Data.SqlClient.SqlConnection.OnError(SqlException exception, Boolean breakConnection)
   at System.Data.SqlClient.SqlInternalConnection.OnError(SqlException exception, Boolean breakConnection)
...
--- Call Stack ---
   ...
   at System.Threading.ThreadHelper.ThreadStart()";
		...
		private const string SECOND_EXCEPTION = @"VersionOne.V1Exception: 
/*parameters*/ declare @p0$ int, @p1$ int, @p2$ varchar(8000), @p3$ bigint, @p4$ varchar(8000)
/*input*/ select @p0$=1419992, @p1$=395766, @p2$='Task', @p3$=9, @p4$='Member'
declare @p0 int,@p1 int,@p2 varchar(8000),@p3 bigint,@p4 varchar(8000)
select @p0=@p0$,@p1=@p1$,@p2=@p2$,@p3=@p3$,@p4=@p4$
...
--- Call Stack ---
   ...
   at System.Threading.ThreadHelper.ThreadStart()";
		...
		[Test]
		public void it_should_extract_first_log()
		{
			var first = _logs.ElementAt(0);
			first.VirtualDirectory.should_be("/Motorola");
			first.OccurredAt.should_be(DateTime.Parse("2011-04-19 22:06:58"));
			first.Version.should_be(new Version("10.2.8001.95"));
			first.Exception.should_be(FIRST_EXCEPTION);
		}

		[Test]
		public void it_should_extract_second_log()
		{
			var second = _logs.ElementAt(1);
			second.VirtualDirectory.should_be("/Qualcomm");
			second.OccurredAt.should_be(DateTime.Parse("2011-04-19 22:07:05"));
			second.Version.should_be(new Version("10.3.0.5"));
			second.Exception.should_be(SECOND_EXCEPTION);
		}
	}
}
